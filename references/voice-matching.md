...
humanizer repo must stay untouched. The voice-reading logic is shared across
the pair on purpose: humanizer reads a voice to rewrite *toward* it,
authenticity-check reads the same voice to measure *deviation from* it. The
reading dimensions are identical; only the use differs, and that framing
lives in this repo's SKILL.md, not in this vendored file.

Sync obligation: when humanizer's criteria change, re-sync this file. Do not
edit the criteria here independently. A fix belongs upstream in humanizer and
is then re-vendored here, otherwise the two repos drift and the pair stops
agreeing.

Reconciliation: these vendored copies and humanizer's originals will be
collapsed into a single shared source of truth when the `voiceprint` product
